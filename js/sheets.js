...
async function fetchSheetTab(tabName) {
  ...
  if (!res.ok) {
    throw new Error(`Could not reach the "${tabName}" tab (status ${res.status}). Make sure the sheet is shared as "Anyone with the link — Viewer."`);
  }
...
/** Renders a 0–5 rating as gold/outline note-star markup. */
